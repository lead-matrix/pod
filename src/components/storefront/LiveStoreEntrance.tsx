  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 flex justify-between px-12 opacity-10 pointer-events-none">
        <div className="w-[1px] h-full bg-gradient-to-b from-transparent via-[#c9a84c] to-transparent" />
        <div className="w-[1px] h-full bg-gradient-to-b from-transparent via-[#c9a84c] to-transparent" />
        <div className="w-[1px] h-full bg-gradient-to-b from-transparent via-[#c9a84c] to-transparent" />
      </div>
      ...
      <div
        className={`absolute inset-y-0 left-0 w-1/2 border-r border-white/5 bg-neutral-950/90 backdrop-blur-2xl transition-transform duration-1000 ease-out flex items-center justify-end pr-8 ${
          entranceState === 'opening' ? '-translate-x-full' : 'translate-x-0'
        }`}
      >
        {/* Left reflection line */}
        <div className="absolute inset-y-0 right-0 w-[1px] bg-gradient-to-b from-white/0 via-white/10 to-white/0" />
      </div>

      <div
        className={`absolute inset-y-0 right-0 w-1/2 border-l border-white/5 bg-neutral-950/90 backdrop-blur-2xl transition-transform duration-1000 ease-out flex items-center justify-start pl-8 ${
          entranceState === 'opening' ? 'translate-x-full' : 'translate-x-0'
        }`}
      >
        {/* Right reflection line */}
        <div className="absolute inset-y-0 left-0 w-[1px] bg-gradient-to-b from-white/0 via-white/10 to-white/0" />
      </div>
      ...
      >
        {/* Live status badge */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 border border-[#c9a84c]/30 text-xs font-semibold text-[#c9a84c] tracking-widest uppercase mb-6 font-mono-label">
        ...
        <p className="text-gray-500 text-sm md:text-base max-w-sm tracking-wide leading-relaxed font-light">
          Step into our immersive live catalog. Real-time runway models, hand-crafted designer silhouettes, and exclusive capsule drops.
        </p>

        {/* Quick status feed */}
        <div className="mt-8 mb-8 flex items-center gap-6 px-4 py-2.5 bg-white/[0.01] border border-white/[0.04] text-[10px] font-mono-label text-gray-500 uppercase tracking-wider">
          <div className="flex items-center gap-1.5">
            <Users className="h-3.5 w-3.5 text-[#c9a84c]" />
            <span><strong className="text-white">{visitors}</strong> inside</span>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full">
          <button
            ...
            className="w-full glass-button-primary justify-center h-12"
          >
          ...
          <button
            ...
            className="w-full sm:w-auto h-12 px-6 border border-white/10 bg-transparent hover:border-[#c9a84c]/50 text-xs font-mono-label uppercase tracking-widest text-gray-400 hover:text-white flex items-center justify-center gap-2 transition-colors active:scale-95"
          >
            {audioEnabled ? (
              <>
                <Volume2 className="h-4 w-4 text-[#c9a84c]" />
                <span>Audio: On</span>
              </>
            ) : (
              <>
        ...
        <p className="text-[9px] font-mono-label text-gray-600 mt-5 uppercase tracking-widest">
          Ambient beats will initialize on entrance
        </p>
      </div>
    </div>
  )
